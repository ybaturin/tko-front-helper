...
class TemplateCreator {
  ...
  _getRenamedCode(fileSource) {
    ...
    try {
      this.log('Start rename code...');

      const tempNameReg = new RegExp(`(^|\\s|,|\\.|\\(|\\)|'|")(${this.templateName})(\\s?|,|\\.|\\(|\\)|'|")`, 'g');
      result = fileSource.replace(tempNameReg, `$1${this._getComponentName()}$3`);
      ...
      result = fileSource.replace(tempConstructorNameReg, `$1${this._getConstructorName()}$3`);
      ...
      result = fileSource.replace(tempCssComponentNameReg, `$1${this._getCssComponentName()}$3`);
