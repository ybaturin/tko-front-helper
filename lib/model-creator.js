...
const fs = require('fs');
const rootAppPath = require('./tko-root-path');
...
class ModelCreator {
  ...
  _renameCodeNames() {
    try {
      ...
      const filePath = path.join(modelsFolder, `${this.modelName}.ts`);
  ...
  _renameFile() {
    try {
      const modelsFolder = this._getModelsFolderPath();
      const modelFilePath = path.join(modelsFolder, 'templateModel.ts');
      const newModelName = path.join(modelsFolder, `${this.modelName}.ts`);
