...
class CardCreator {
  ...
  _renameFiles() {
    try {
      ...
      const templatePath = path.join(this._getCurrFolderPath(), TEMPLATE_NAME);
      const list = fs.readdirSync(templatePath);
      
      list.forEach(fileName => {
        const oldName = path.join(templatePath, fileName);
        const newName = path.join(templatePath, this._getComponentName());
  ...
  _copyTemplateFolder(callback) {
    ...
    const target = path.join(this._getCurrFolderPath(), TEMPLATE_NAME);
    ncp(TEMPLATE_CARD_PATH, target, (err) => {
      if (err) {
